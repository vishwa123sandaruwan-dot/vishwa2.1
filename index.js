...
async function connectToWA() {
  console.log("Connecting vishwa 2.0 🧬...");
  ...
  danuwa.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log('✅ vishwa 2.0 connected to WhatsApp');
  ...
  danuwa.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (isCmd) {
      ...
      if (cmd) {
        ...
        // AI Chat Trigger - respond if 'vishwa' is mentioned or message is a reply to the bot
        const isMentioned = body.toLowerCase().includes("vishwa");
        const isReplyToBot = mek.message?.extendedTextMessage?.contextInfo?.participant === botNumber2;
        
        if (isMentioned || isReplyToBot) {
            try {
                const aiPrompt = body.replace(/vishwa/gi, "").trim() || "Hi";
                const { fetchJson } = require('./lib/functions');
                const aiData = await fetchJson(`https://api.giftedtech.my.id/api/ai/gpt4?apikey=gifted&q=${encodeURIComponent(aiPrompt)}`);
                
                if (aiData && aiData.results) {
                    await reply(aiData.results + "\n\n> *vishwa 2.0 AI* 🧬");
                }
            } catch (err) {
...
app.get("/", (req, res) => {
  res.send("Hey, vishwa 2.0 started✅");
});
